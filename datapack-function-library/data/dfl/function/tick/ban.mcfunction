## TODO
tp 0.0 0.0 0.0
gamemode adventure
title @s times 0 70 20
title @s title {"type":"translatable","translate":"tick.ban.dfl.ban","fallback":"您已被此服务器软封禁","color":"red","bold":true}
title @s subtitle [{"type":"translatable","translate":"tick.ban.dfl.from","fallback":"通过数据包","color":"gray"},{"text":" datapack function library","color":"gray"}]
effect give @s blindness 60 255
effect give @s slowness 60 255
effect give @s mining_fatigue 60 255
effect give @s resistance 60 255
effect give @s regeneration 60 255
effect give @s unluck 60 255
effect give @s glowing 60 255
effect give @s weakness 60 255
